...
public class FileGenerator {
     ...
     * @throws IOException
     */
    public static void createAll(String file_path, int total_num) throws IOException {
        File f_all = new File(file_path); // todo 变量变遵循规范
        f_all.createNewFile();
        BufferedWriter out = new BufferedWriter(new FileWriter(f_all));
        writeTitle(out); // todo title可配置
        for (int i = 0; i < total_num; i++) {
            //RandomPersonGenerator newPerson = new RandomPersonGenerator();
            writeEachInfo(out, RandomPersonGenerator.createSingleRandomPerson());
        }
        out.close();
    }
     ...
     * @throws IOException
     */
    public static void createEach(String file_path, int group_num) throws IOException {
     ...
     * @throws IOException
     */
    private static void writeEachInfo(BufferedWriter out, Person newPerson) throws IOException {
        //out.write(Joiner.on(", ").skipNulls().join(newPerson.getId(), newPerson.getName(), newPerson.getSex()));
        out.write(newPerson.getId() + ", " + newPerson.getName() + ", " + newPerson.getSex() + ", " + newPerson.getAge() + ", " + newPerson
                .getDesc() + ", "
    ...
    public static void main(String[] args) throws IOException {
        ...
        createAll(file_path, 100);
        createEach(file_path, 10);
    }
}
